...
async def _generate_opener_in_background(
    ...
    *,
    ...
    try:
        ...
        if not _api_key or _api_key == "placeholder_key":

            logger.info(

                "[START-BG] session=%s no API key — keeping fallback opener",
...
Begin the interview by introducing yourself briefly as the {target_company} interviewer and asking the first question appropriate for a {experience_level} candidate applying for a {job_type or "full time job"} {role or "Software Engineer"} position in a {interview_type} loop."""
        ...
        session_obj.question_source = "llm"

        session_db.commit()

        logger.info("[START-BG] session=%s opener swapped (question_source=llm)", session_id)
...
async def start_interview(
    ...
    try:
        ...
        # If it is a coding interview, set up the fallback questions and trigger background customization

        if "coding" in interview_type.lower().strip():

            from sqlalchemy.orm.attributes import flag_modified

            from app.api.coding import _generate_coding_challenges_in_background, FALLBACK_CODING_CHALLENGES
...
Ensure the JSON is valid."""
    ...
    next_question = _SCORING_UNAVAILABLE_NEXT_QUESTION
    ...
    for _attempt in range(2):
        ...
        logger.info(
            ...
            "[NIM-Score] request model=%s session=%s round=%d attempt=%d",

            _model, payload.session_id, user_message_count, _attempt + 1,
        ...
        try:
            ...
            logger.info(
                ...
                "[NIM-Score] response latency_ms=%d session=%s round=%d",

                _latency_ms, payload.session_id, user_message_count,
            ...
            logger.error(
                ...
                "[NIM-Score] timeout latency_ms=%d session=%s round=%d attempt=%d",

                _latency_ms, payload.session_id, user_message_count, _attempt + 1,
            ...
            logger.error(
                ...
                "[NIM-Score] rate_limited latency_ms=%d session=%s round=%d attempt=%d",
